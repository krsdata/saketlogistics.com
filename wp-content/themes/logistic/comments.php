<div id="comments">
	...
	<?php if(!empty($_SERVER['SCRIPT_FILENAME']) && 'comments.php' == basename($_SERVER['SCRIPT_FILENAME'])) : ?>
	    <?php die(__('Please do not load this page directly. Thanks and have a great day!', 'vp_textdomain')); ?>
	<?php endif; ?>
	...
	<?php if($comments) : ?>
		<h3><?php comments_number(__('No comments', 'vp_textdomain'), __('One comment', 'vp_textdomain'), __('% comments', 'vp_textdomain')); ?></h3>
	    <ol>
	...
	<?php else :
	    ...
	    /*echo '<p>';
		_e('No comments yet.', 'vp_textdomain');
		echo '</p>';*/
	endif; ?>
			...
			<?php if(get_option('comment_registration') && !$user_ID) : ?>
                ...
                <?php 
					$comment_form_fields = array(
						'author' => 
							'<p class="half-input">
								<label for="author" class="none">'. __('Name', 'vp_textdomain') .' <small>' . ($req ? "<span class='required'>*</span>" : "") .'</small></label>
						...
						'url' =>
							'<p class="half-input">
								<label for="url" class="none">'. __('Website', 'vp_textdomain') .'</label>
								<input type="text" name="url" id="url" value="' .$comment_author_url .'" placeholder="'. __('Website', 'vp_textdomain') .'" size="22" tabindex="2" />
							</p>',							
						'email' => 
							'<p class="full-input">
								<label for="email" class="none">'. __('Mail (will not be shared)', 'vp_textdomain') .' <small>' . ($req ? "<span class='required'>*</span>" : "") .'</small></label>
					...
					comment_form( array(
						...
						'comment_field' => '
							<p>
								<label for="comment" class="none">'. __('Comment', 'vp_textdomain') .'</label>
