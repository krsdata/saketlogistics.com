                    ...
                    <div class="ozy-masonry-gallery">
						<?php
                            foreach(ozy_grab_ids_from_gallery() as $attachment_id) {
                                ...
                                echo '<a href="'. esc_attr($attachment->guid) .'" class="fancybox" rel="gallery" title="'. esc_attr($attachment->post_title) .'"><img src="'. $thumb_image[0] .'" alt=""/><span><h5 class="title">'. $attachment->post_title .'</h5></span></a>';
								echo '</div>';
                            }
                        ?>
                    </div>
