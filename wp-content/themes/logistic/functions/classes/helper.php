		...
		function language_selector_flags() {
			if(function_exists("icl_get_languages") && defined("ICL_LANGUAGE_CODE") && defined("ICL_LANGUAGE_NAME")){
				...
				if(!empty($languages)){
					...
					echo '<ul id="footer-language-switcher-option" class="main-bg-color">' . PHP_EOL;
		...
		*/
		function newer_older_post_navigation_post($padding=false) {
            ...
            <div class="newer-older<?php echo $padding ? ' newer-older-blog' : '' ?>">
                <?php 
                    previous_post_link('%link', __('&larr; Previous Post', 'vp_textdomain')); 
                    if((int)ozy_get_option('page_blog_list_page_id')>0) {
                        echo '<a href="'. get_permalink(ozy_get_option('page_blog_list_page_id')) .'">' . __('All Posts', 'vp_textdomain') . '</a>';
                    }
                    next_post_link('%link', __('Next Post &rarr;', 'vp_textdomain')) 
                ?>
            </div><!--.newer-older-->               
            <?php			
		}	
