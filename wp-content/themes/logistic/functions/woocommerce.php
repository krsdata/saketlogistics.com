...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	ob_start();
	?>
    <a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class="cart-contents"><i class="oic-simple-line-icons-52"></i>&nbsp;&nbsp;<span><?php _e('SHOPPING BAG:&nbsp;&nbsp;', 'vp_textdomain');echo $woocommerce->cart->get_cart_total();?></span></a>
	<?php
			...
			if(count($product_attachment_ids)>1) {
				...
				foreach($product_attachment_ids as $attachment_id) {
					...
					if(isset($thumb_img[0])) {
						$output .= sprintf('<div class="item"><img class="lazyOwl" data-src="%s" src="'. OZY_BASE_URL .'images/blank-large.gif" alt=""/></div>', esc_url($thumb_img[0]));
					}
				}
				$output .= '</div>';
			}else{
