...
if(jQuery) (function($) {
	...
	function doChange(input, hex, opacity) {
		...
		input.change(); //added by ozythemes, better warn input field about value change
...
/*initialize mini color picker, if installed*/
/*jQuery(document).ready(function($) {	
	if (jQuery().minicolors) {
		jQuery(document).ajaxComplete(function() { //after widgets save, we have to re-call color colorbox
			jQuery('input.ozy-simple-color-picker').minicolors('destroy').minicolors();
		});
		jQuery('input.ozy-simple-color-picker').minicolors();
	}
});*/
/*disabled this block, because I am calling function when settings being loaded in admin.js*/