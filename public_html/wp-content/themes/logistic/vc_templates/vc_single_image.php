...
if ( $img == NULL ) $img['thumbnail'] = '<img class="vc_img-placeholder vc_single_image-img" src="' . vc_asset_url( 'vc/no_image.png' ) . '" />'; //' <small>'.esc_attr__('This is image placeholder, edit your page to replace it.', 'js_composer').'</small>';
...
if($img_animated === 'yes') {
	...
	if($img_animation !== '') {	
		$css_class .= ' ' . $img_animation;
	}
}

if($zoom_on_hover === 'yes') {
	$css_class .= ' zoom';
}

if($lightbox === 'yes') {
	$css_class .= ' single-image-fancybox';
}
