			...
			if ($shop_page_id > 0) { 
				...
				if(ozy_get_metabox('hide_title', 0, $shop_page_id) !== '1') {
					$ozy_data->_page_custom_page_title = 
						...
						ozy_get_metabox('use_custom_title_group.0.ozy_logistic_meta_page_custom_title', '', $shop_page_id) : 
						get_the_title($shop_page_id);
					$ozy_data->_page_custom_page_sub_title = 
						ozy_get_metabox('use_custom_title_group.0.ozy_logistic_meta_page_custom_sub_title', '', $shop_page_id);
				}else{
				...
				if(is_search()) {
					...
					if(isset($post->ID)) {
						$ozy_data->_page_custom_page_title = 
							...
							ozy_get_metabox('use_custom_title_group.0.ozy_logistic_meta_page_custom_title', '') : get_the_title($post->ID);
						$ozy_data->_page_custom_page_sub_title = 
							ozy_get_metabox('use_custom_title_group.0.ozy_logistic_meta_page_custom_sub_title', '', $post->ID);	
					}
				}
			}
			...
			if (is_single() && get_post_type() === 'post' ) {
				if ( have_posts() ) while ( have_posts() ) : the_post();
					...
					if ( has_post_thumbnail() ) { 
						...
						if(isset($src[0])) {
							echo '<div class="featured-thumbnail-header" style="background-image:url('. esc_url($src[0]) .')">';
							echo '	<div>';
